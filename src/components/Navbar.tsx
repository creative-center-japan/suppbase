// healthy-site\src\components\Navbar.tsx
...
export default function Navbar() {
  ...
  // 外側クリック & Esc で閉じる
  useEffect(() => {
    if (!isJapanOpen) return;

    const handleDown = (e: MouseEvent | TouchEvent) => {
      const el = jpWrapRef.current;
      if (!el) return;
    ...
    const handleEsc = (e: KeyboardEvent) => {
      if (e.key === "Escape") setJapanOpen(false);
    };
        ...
        <div className="hidden sm:flex space-x-6 text-sm font-medium items-center">
          ...
          <div className="relative" ref={jpWrapRef}>
            ...
            {isJapanOpen && (
              <div
                role="menu"
                className="absolute right-0 bg-white border rounded shadow-md mt-2 w-48 z-50"
              >
                <ul className="text-sm text-gray-700">
                  <li>
                    <Link
                      href="/rankings/japan/protein"
                      onClick={() => setJapanOpen(false)}
                      className={`block px-4 py-2 hover:bg-gray-100 ${
                        pathname === '/rankings/japan/protein' ? 'bg-green-100 font-bold' : ''
                      }`}
                    >
                      プロテイン
                    </Link>
                  ...
                  <li>
                    <Link
                      ...
                      onClick={() => setJapanOpen(false)}
                      className={`block px-4 py-2 hover:bg-gray-100 ${
                        pathname === '/rankings/japan/supplements' ? 'bg-green-100 font-bold' : ''
                      }`}
                    >
                      サプリメント
                    </Link>
                  ...
                  <li>
                    <Link
                      href="/rankings/japan/foods"
                      onClick={() => setJapanOpen(false)}
                      className={`block px-4 py-2 hover:bg-gray-100 ${
                        pathname === '/rankings/japan/foods' ? 'bg-green-100 font-bold' : ''
                      }`}
                    >
                      フード
                    </Link>
                  ...
                  <li>
                    <Link
                      href="/rankings/japan/others"
                      onClick={() => setJapanOpen(false)}
                      className={`block px-4 py-2 hover:bg-gray-100 ${
                        pathname === '/rankings/japan/others' ? 'bg-green-100 font-bold' : ''
                      }`}
                    >
                      その他
                    </Link>
